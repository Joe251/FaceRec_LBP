...
//const char *faceCascadeFilename = "haarcascade_frontalface_alt2.xml";  // Haar face detector.
...
int numread(string filename);
string dataread(string filename, int faceid);
void csv_write(int photo_id);
void input_information();
void learning_faces(const vector<Mat> preprocessedFaces, const vector<int> faceLabels, const string facerecAlgorithm, Ptr<FaceRecognizer> model);
...
void onMouse(int event, int x, int y, int, void*)
{
	...
	if (isPointInRect(pt, m_rcBtnAdd)) {
		string name;
		char filename[] = "F:/facedata/face_csv.txt";
		int category;
		newface = true;
		face_count = 0;

		fstream f("personaldata.txt", ios::out | ios::in);
		f.seekg(0, ios::end);
		//扫描txt文件，找到最后label值。///////////////////////////////////////////////////////
		if (first_time){
			initialnum = numread(filename) + 1;
			first_time = false;
		}
		std::thread t(input_information);
		t.detach();
	}
...
void input_information(){
	...
	cin >> name;
	cout << "What you are?(0-normal,1-elder,2-children)" << endl;
	cin >> category;
	f << endl << initialnum + m_numPersons << "," << name << "," << category;
	f.close();
	cout << "Your name is " << name << endl << "You are a " << (category == 0 ? "normal people" : (category == 1 ? "elder" : "children"))
		<< endl << "Your id is:" << initialnum + m_numPersons << endl;
	cout << "User clicked [Add Person] button when numPersons was " << m_numPersons + initialnum << endl;
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
	...
	fstream _file;
	_file.open("LBPHface.xml", ios::in);
	if (_file)
	{
		model = createLBPHFaceRecognizer(2,16);
		model->load("LBPHface.xml");
		_file.close();
	...
	while (true) {
		...
		//\CAռ\AF\C1\B3
			...
			if (face_count != 10){
				if (gotFaceAndEyes) {
					...
					*/
					...
					if ((imageDiff > CHANGE_IN_IMAGE_FOR_COLLECTION) && (timeDiff_seconds > CHANGE_IN_SECONDS_FOR_COLLECTION)) {
						...
						Mat mirroredFace;
						Mat result_Face;
						result_Face = tan_triggs_preprocessing(preprocessedFace);

						flip(result_Face, mirroredFace, 1);

						// Add the face images to the list of detected faces.
						preprocessedFaces.push_back(result_Face);
						preprocessedFaces.push_back(mirroredFace);
						cout << "debug initialnum:" << initialnum << endl;
						cout << "debug m_selectedPerson:" << m_selectedPerson << endl;
						faceLabels.push_back(initialnum + m_selectedPerson);
						faceLabels.push_back(initialnum + m_selectedPerson);
						...
						//不允许出现0
						(save_face = ((preprocessedFaces.size() / 2) % 10)) == 0 ? save_face = 10 : NULL;

						// Show the number of collected faces. But since we also store mirrored faces, just show how many the user thinks they stored.
						cout << "Saved face " << save_face << " for person " << initialnum + m_selectedPerson << endl;

						face_count++;

						csv_write(save_face);
						char string[30];
						sprintf(string, "F:/facedata/s%d", initialnum + m_selectedPerson + 1);
						if (_access(string, 0) == -1){
							_mkdir(string);
						}
						/*char filename[]="photo";
						sprintf(string,"photo/%d",m_selectedPerson);
						...
						//cv::imwrite("./photo/"+toString(m_selectedPerson)+"/"+toString(preprocessedFaces.size()/2)+".pgm", preprocessedFace);
						cv::imwrite("F:/facedata/s" + toString(initialnum + m_selectedPerson + 1) + "/" + toString(save_face) + ".pgm", preprocessedFace);
				...
				cout << "Adding finished!Please click the image or Add Person!" << endl;
			}
		...
		//ѵ\C1\B7
			...
			if (haveEnoughData) {
				m_mode = MODE_RECOGNITION;
				std::thread t(learning_faces, preprocessedFaces, faceLabels, facerecAlgorithm, model);
				t.detach();
			}
			else {
		...
		//ʶ\B1\F0
			...
			if (gotFaceAndEyes){
				...
				compareHist(preprocessedFace, reconstructedFace, compare_method);
				*/
				Mat srcImage;
				double distance;
				string outputStr;
				preprocessedFace = tan_triggs_preprocessing(preprocessedFace);
				model->set("threshold", 45);
				identity = model->predict(preprocessedFace);
				//cout << distance << endl;
				if (identity == -1)
				{
					cout << "Unknow!" << endl << endl;
				}
				else
				{
					string personaldata;
					...
					stringstream ss;
					int category;
					cout << "debug identity:" << identity << endl;
					outputStr = toString(identity);
					personaldata = dataread("personaldata.txt", identity);
					name = personaldata.substr(0, personaldata.find_first_of(','));
					ss << personaldata.substr(personaldata.find_first_of(',') + 1, personaldata.size());
					ss >> category;
					cout << "Identity: " << outputStr << endl;
					cout << "name:" << name << endl;
					cout << "category:" << (category == 0 ? "normal people" : (category == 1 ? "elder" : "children")) << endl << endl;
				}
...
void learning_faces(const vector<Mat> preprocessedFaces, const vector<int> faceLabels, const string facerecAlgorithm, Ptr<FaceRecognizer> model){
	learnCollectedFaces(preprocessedFaces, faceLabels, facerecAlgorithm, model);
...
int numread(string filename){
	...
	int number;
	if (in){
		while (getline(in, linename)) // line中不包括每行的换行符  
		{
		}
	}
	ss << linename.substr(linename.find_first_of(';') + 1, linename.size());
	ss >> number;
	return number;
}
string dataread(string filename, int faceid){
...
void csv_write(int photo_id){
	fstream in("F:/facedata/face_csv.txt", ios::out | ios::in);
	in.seekg(0, ios::end);
	in << endl << "F:/facedata/s" << initialnum + m_selectedPerson + 1 << "/" << photo_id << ".pgm" << ";" << initialnum + m_selectedPerson;
}
